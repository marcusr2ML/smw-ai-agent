...
agent = AgentVAE(input_dims=env.observation_space.shape, num_actions=env.action_space.n)


if  SHOULD_TRAIN:
    folder_name = ""         
    kpt_name = "VAE_jumps_runs.pth"            
    
    agent.load_model(os.path.join("model_save/", folder_name, ckpt_name))
    agent.epsilon = 3.0
    agent.eps_min = 05.0
